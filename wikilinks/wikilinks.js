...
function getArticle(index, name) {
  let article = index[name] || {name, children: [], terminal: 0, maxDepth: 0};
  index[name] = article;
  return article;
}
...
function getArticles() {
  let index = {};
  for (let name in articleNames) {
    let parentName = articleNames[name];
    let article = getArticle(index, name);
    let parent = getArticle(index, parentName);
...
function findChain(article, chain) {
  if (chain.includes(article)) {
    let last = chain[chain.length - 1];
    last.terminal++;
    return chain;
  }
  chain.push(article);
  return findChain(article.parent, chain);
}

function findChains(articles) {
  articles
    .filter((article) => article.children.length == 0)
    .forEach((article) => {
      article.chain = findChain(article, []);
      let depth = article.chain.length;
      article.chain.forEach((a) => a.maxDepth = Math.max(a.maxDepth, depth));
    });
}
...
function printArticle(article, depth) {
  ...
  let uri = encodeURI(article.name);
  let link = "https://en.wikipedia.org/wiki/" + uri;
  let style = article.children.length ? "" : "link";
  main.innerHTML += `<a name="${uri}" />${dots}<a href="${link}" class="${style}">${article.name}</a><br>`;
  article.printed = true;
  article.children
    .sort((a1, a2) => a2.maxDepth - a1.maxDepth)
    .forEach((child) => printArticle(child, depth + 1));
}
